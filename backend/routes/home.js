...
router.get("/:user/:profile/home", async function (req, res) {
  ...
  if (!perfile) return res.status(400);
...
router.delete("/:user/:profile/megusta_dlt", async function (req, res) {
  try {
    ...
    perfilM.likeList = perfilM.likeList.filter(
      (element) => element.toString() !== videoObjectId
    );
    await perfilM.save();
...
router.delete("/:user/:profile/pendientes_dlt", async function (req, res) {
  try {
    ...
    perfilM.pendienteList = perfilM.pendienteList.filter(
      (element) => element.toString() !== videoObjectId
    );
    await perfilM.save();
...
router.put("/:user/:profile/visto_add", async function (req, res) {
  try {
    let perfil = req.params.profile;
    let user = req.params.user;
    const videoId = req.body.videoId;
...
router.delete("/:user/:profile/visto_dlt", async function (req, res) {
    try {
      const perfil = req.params.profile;
      const user = req.params.user;
      const videoId = req.body.videoId;
      ...
      if (
        ...
        return res.status(200).json({
          message: 'El video no está en la lista de "vistoList"',
        });
      }
  
      perfilM.vistoList = perfilM.vistoList.filter(
        (element) => element.toString() !== videoObjectId
      );
      await perfilM.save();
  
      res.status(200).json({
        message: 'Video eliminado de la lista de "vistoList"',
      });
    } catch (error) {
      res.status(500).json({
